...
```{r}
#| warning: false
# Define the rquery.cormat function
...
rquery.cormat <- function(x, type = c('lower', 'upper', 'full', 'flatten'),
  ...
  # 1. Helper function to compute the matrix of correlation p-values
  cor.pmat <- function(x, ...) {
    mat <- as.matrix(x)
    res <- rcorr(mat, ...)
    return(res$P)
  ...
  # 2. Helper function to print upper/lower triangles or full matrix
  clean.cormat <- function(cormat, type = c('lower', 'upper', 'full')) {
    type <- match.arg(type)
  ...
  # 3. Helper function to flatten the matrix
  ...
  if (graph) {
    ...
    if (is.null(col)) {
      if (graphType == "correlogram") {
        col <- colorRampPalette(c("#BB4444", "#EE9999", "#FFFFFF", "#77AADD", "#4477AA"))(200)
    ...
    if (graphType == "correlogram") {
      corrplot(cormat, type = ifelse(type == "flatten", "full", type),
  ...
  # Format and return the final matrix/table
  if (type == "flatten") {
    return(flattenCorrMatrix(cormat, pmat))
  } else {
    return(clean.cormat(cormat, type))
  }
}
...
# to get the function (also defined above)
...
::: {style="text-align: justify"}
## Identifying Infuential Observations

      Influential observations might affect the estimates of the regression equations. Therefore, sometimes we need to identify them and run the regression model after excluding them. @fig-influ-r and @fig-influ-py show the influential observations in a given model. 
...
      A general rule of thumb is that a Cook's distance greater than 4/𝑛
 (where 𝑛 is the total number of observations) indicates a potentially influential observation.
...
```{r}
#| warning: false
#| fig-cap: Influential Observations (R)
#| label: fig-influ-r
...
```{python}
#| warning: false
#| fig-cap: Influential Observations (Python)
#| label: fig-influ-py
import statsmodels.api as sm
import matplotlib.pyplot as plt
import seaborn as sns
...
::: {style="text-align: justify"}
      Alternatively, we can identify a certain number (e.g., 10) of influential observations by running the following code - 
::: 
...
data.frame(Index = top_10, Cook_Distance = cooks_d[top_10])
```
...
# Display the top 10 influential observations
print(top_10_influential[['index', 'cooks_d', 'hat_diag', 'student_resid']])
```
