...
     Using terminal on Windows and Mac, we can connect to the database. On Windows, we use command prompt (`cmd`) and on Mac we use `Terminal App` to use terminal. 
...
::: {style="text-align: justify"}
     To use the command prompt (`cmd`) on Windows to connect to PostgreSQL database, you need to use the following steps -
...
::: {style="text-align: justify"}
     To use the Terminal App on Mac to connect to PostgreSQL database, you need to use the following steps -

-   Step \# 01 - Press `Command + Spacebar`, then type `Terminal` and hit Enter button.

![Search Bar on Mac](images/terminal_mac.webp)

-   Step \# 02 - type `cd /Library/PostgreSQL/16/bin` on the terminal and hit enter on the keyboard

-   Step \# 03 - type `./psql -U postgres `
...
     Sometimes, you might need to upload a database to PostgreSQL. Therefore, you should know how you can upload the database to PostgreSQL. It is showed below how to upload a database to PostgreSQL both in Windows and Mac.
...
/Library/PostgreSQL/16/bin/pg_restore -U postgres -d my_database  K:/my_database_upload.tar
```

     In the above line of code above - `/Library/PostgreSQL/16/bin/pg_restore -U postgres -d my_database  K:/my_database_upload.tar`- "`my_database`" is the name of the database that you created on the PostgreSQL and "`K:/my_database_upload.tar`" is the path^[To get the path of a folder on Mac- first, select the folder and right click your mouse; then, click "Option" button on the keyboard and finally click "Copy ... as pathname"] of the database to be uploaded.
...
Sys.setenv('RETICULATE_PYTHON' = "C:/Users/mshar/OneDrive - Southern Illinois University/ANALYTICS_FOR_ACCOUNTING_DATA/accounting_analytics_book/Scripts/python.exe")
```
...
# installing ibis - pip install "ibis-framework[postgres]"
import ibis
import getpass
import os 
```
...
1.  Crate a `.env` file in your working directory. You can create the `.env` file using notepad, but note that the extension of the file `.txt` must be deleted. Once the `.env` file is created, then save your password as you did in R enviornment. For example - save the line - `POSTGRESS_PASSWORD="YOURPASSWORD"`, where replace the word YOURPASSWORD with your own password.
...
[^data_management-1]: OLTP Database Management Systems are databases designed to handle a large number of short, fast, day-to-day transactions reliably and concurrently. They are the backbone of operational applications like banking systems, e-commerce platforms, reservation systems, and ERP software.
